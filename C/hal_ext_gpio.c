...
static char *hal_id = "hal_ext_gpio";
...
int rtapi_app_main(void)
{
    ...
    comp_id = hal_init("hal_ext_gpio");
    if (comp_id < 0) { rtapi_print_msg(RTAPI_MSG_ERR, "HAL_PI_GPIO: ERROR: hal_init() failed\n");
	return -1;
    }
    ...
    for (n = 0; n < npins; n++) {
      ...
      if (dir_map & RTAPI_BIT(n)) {
        ...
        if ((retval = hal_pin_bit_newf(HAL_IN, &port_data[n], comp_id, "%s.pin-%02d-out", hal_id, pinno)) < 0) {
          ...
          rtapi_print("BIT out %d %d\n", n, retval);
        }
      } else {
        bcm2835_gpio_fsel(gpios[n], BCM2835_GPIO_FSEL_INPT);
        if ((retval = hal_pin_bit_newf(HAL_OUT, &port_data[n], comp_id, "%s.pin-%02d-in",hal_id, pinno)) < 0) {
          break;
        } else {
          rtapi_print("BIT in %d %d\n", n, retval);
        }
      }
    }
    if (retval < 0) {
    ...
    retval = hal_export_funct(write_buffer, write_port, 0, 0, 0, comp_id);
    if (retval < 0) {
      rtapi_print("failed write\n");
      rtapi_print_msg(RTAPI_MSG_ERR, "HAL_PI_GPIO: ERROR: write funct export failed\n");
      hal_exit(comp_id);
      return -1;
    }
    char read_buffer[strlen(hal_id) + 6];
    sprintf(read_buffer, "%s.read", hal_id);
